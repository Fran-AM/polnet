...
def widgets_visualize_pdb():
    ...
    """
    text_protein_widget = widgets.Text(
        description="Enter protein name:", value="1bxn", placeholder="1bxn"
    )
    text_protein_widget.style.description_width = "initial"
    text_protein_widget.layout.width = "450px"

    save_button = widgets.Button(description="Save")
    file_destination_widget = FileChooser(
        os.getcwd(), title="Select the path and tha name for the file"
    )

    display(
        widgets.HBox([text_protein_widget, save_button]),
...
def widgets_convert_to_mrc():
    ...
    title = widgets.Label(
        value="Introduce a the files you want to create the density map"
    )
    files_button = widgets.Button(
        description="Select files you want to convert"
    )
    files_button.layout.width = "200px"

    dropdown = widgets.Dropdown(value=None, layout={"width": custom_width})

    apix_widget = widgets.BoundedFloatText(
        value=10, min=0, step=0.001, description="VSIZE:"
    )
    apix_widget.style.description_width = "initial"

    res_widget = widgets.BoundedFloatText(
        value=30, min=0, step=1, description="RESOLUTION:"
    )
    res_widget.style.description_width = "initial"

    offset_widget = widgets.BoundedFloatText(
        value=20, min=0, step=1, description="OFFSET:"
    )
    offset_widget.style.description_width = "initial"

    het_widget = widgets.Checkbox(
        value=False, description="Include HETATM atoms"
    )
    ...
    use_model_checkbox = widgets.Checkbox(
        value=False, description="Use Model Number:"
    )
    use_model_checkbox.style.description_width = "initial"

    model_widget = widgets.BoundedIntText(
        value=None, min=1, step=1, description="Model Number:", disabled=True
    )
    ...
    file_destination_widget = FileChooser(
        os.getcwd(),
        show_only_dirs=True,
        title="Select where you want to save the files:",
    )
...
def widgets_mmolecules_params():
    ...
    custom_width = "550px"
    title = widgets.Label(
        value="Introduce the protein you want to create the macromolecule"
    )
    file_destination_widget = FileChooser(
        os.getcwd(), title="Select where you want to save the files"
    )
    ...
    # Crear widgets
    mmer_id_widget = widgets.Text(
        description="MMER_ID:", value="pdb_1bxn", placeholder="pdb_1bxn"
    )
    mmer_id_widget.style.description_width = "initial"

    pmer_l_widget = widgets.BoundedFloatText(
        value=1.2, min=0, step=0.0001, description="PMER_L:"
    )
    pmer_l_widget.style.description_width = "initial"

    pmer_l_mex_widget = widgets.BoundedFloatText(
        value=3000, min=0, step=0.0001, max=100000000, description="PMER_LMEX:"
    )
    pmer_l_mex_widget.style.description_width = "initial"

    pmer_occ_widget = widgets.BoundedFloatText(
        value=0.5, min=0, step=0.0001, max=1, description="PMER_OCC:"
    )
    pmer_occ_widget.style.description_width = "initial"

    pmer_over_tol_widget = widgets.BoundedFloatText(
        value=0.01, min=0, step=0.0001, max=1, description="PMER_OVER_TOL:"
    )
    pmer_over_tol_widget.style.description_width = "initial"

    pmer_reverse_normals = widgets.Checkbox(
        value=False, disabled=True, indent=False
    )
    pmer_label = widgets.Label(
        "Marks if you want to reverse membrane protein normals"
    )
    membrane_option = widgets.Checkbox(
    ...
    widgets_and_labels = [
        widgets.HBox(
            [mmer_id_widget, widgets.Label(value=labels["mmer_id_widget"])]
        ),
        widgets.HBox(
            [pmer_l_widget, widgets.Label(value=labels["pmer_l_widget"])]
        ),
        widgets.HBox(
            [
                pmer_l_mex_widget,
                widgets.Label(value=labels["pmer_l_mex_widget"]),
            ]
        ),
        widgets.HBox(
            [pmer_occ_widget, widgets.Label(value=labels["pmer_occ_widget"])]
        ),
        widgets.HBox(
            [
                pmer_over_tol_widget,
                widgets.Label(value=labels["pmer_over_tol_widget"]),
            ]
        ),
    ]
    display(
        ...
        widgets_top,
        widgets_row,
        widgets.VBox(widgets_and_labels),
        widgets_row2,
        file_destination_widget,
        save_button,
    )
...
def widgets_create_axis():
    ...
    title = widgets.Label(
        value="Introduce a the files you want to create an axis"
    )
    select_files_button = widgets.Button(
        description="Select Files", layout=widgets.Layout(margin="0 5px 0 0")
    )
    dropdown_1 = widgets.Dropdown(value=None, layout={"width": custom_width})
    ...
    mb_size_widget = widgets.BoundedFloatText(
        value=50, min=0, step=0.001, max=100000000, description="MB size:"
    )
    mb_size_widget.style.description_width = "initial"

    zaxis_rad_widget = widgets.BoundedFloatText(
        value=5, min=0, step=0.001, description="Zaxis rad:"
    )
    zaxis_rad_widget.style.description_width = "initial"

    vsize_widget = widgets.BoundedFloatText(
        value=10, min=0, step=0.001, description="Voxel size:"
    )
    vsize_widget.style.description_width = "initial"

    save_button = widgets.Button(
        description="Save", layout=widgets.Layout(margin="8px 0 0 0")
    )

    scale_factor_widget = widgets.BoundedFloatText(
        value=1.5, min=1, step=0.1, max=3, description="Scale factor:"
    )
    ...
    widgets_and_labels = [
        widgets.HBox(
            [mb_size_widget, widgets.Label(value=labels["mb_size_widget"])]
        ),
        widgets.HBox(
            [zaxis_rad_widget, widgets.Label(value=labels["zaxis_rad_widget"])]
        ),
        widgets.HBox(
            [vsize_widget, widgets.Label(value=labels["vsize_widget"])]
        ),
        widgets.HBox(
            [
                scale_factor_widget,
                widgets.Label(value=labels["scale_factor_widget"]),
            ]
        ),
    ]
    ...
    display(
        title,
        widgets.HBox([select_files_button, dropdown_1]),
        widgets.VBox(widgets_and_labels),
        file_destination_widget,
        save_button,
    )
...
def widgets_align():
    ...
    title = widgets.Label(
        value="Introduce a reference membrane membrane and a protein:"
    )
    ...
    display(
        title,
        widgets.HBox([upload_protein_button, dropdown_1]),
        widgets.HBox([upload_axis_button, dropdown_2]),
        file_destination_widget,
        vsize_widget,
        exec_button,
    )
...
def widgets_membrane_params():
    ...
    mb_type_widget = widgets.Dropdown(
        options=options_mb_type, value="sphere", description="MB_TYPE:"
    )
    mb_type_widget.style.description_width = "initial"

    mb_occ_widget = widgets.FloatRangeSlider(
        value=[0.001, 0.003],
        min=0,
        max=100,
        step=0.0001,
    ...
    mb_thick_rg_widget = widgets.FloatRangeSlider(
        value=[25, 45],
        min=20,
        max=50,
        step=1,
        readout_format=".3f",
        description="MB_THICK_RG:",
    )
    mb_thick_rg_widget.style.description_width = "initial"
    mb_thick_rg_widget.layout.width = "350px"

    mb_layer_s_rg_widget = widgets.FloatRangeSlider(
        value=[0.5, 2],
        min=0.1,
        max=3,
        step=0.1,
        readout_format=".3f",
        description="MB_LAYER_S_RG:",
    )
    mb_layer_s_rg_widget.style.description_width = "initial"
    mb_layer_s_rg_widget.layout.width = "350px"

    mb_max_ecc_widget = widgets.BoundedFloatText(
        value=0.75, min=0, step=0.0001, description="MB_MAX_ECC:"
    )
    mb_max_ecc_widget.style.description_width = "initial"

    mb_over_tol_widget = widgets.BoundedFloatText(
        value=1e-9, min=0, step=0.00000000001, max=1, description="MB_OVER_TOL:"
    )
    mb_over_tol_widget.style.description_width = "initial"

    mb_min_rad_widget = widgets.BoundedFloatText(
        value=75, min=0, step=0.0001, max=100000000, description="MB_MIN_RAD:"
    )
    ...
    file_destination_widget = FileChooser(
        os.getcwd(), title="Select destination path"
    )
    ...
    widgets_and_labels = [
        widgets.HBox(
            [mb_type_widget, widgets.Label(value=labels["mb_type_widget"])]
        ),
        widgets.HBox(
            [mb_occ_widget, widgets.Label(value=labels["mb_occ_widget"])]
        ),
        widgets.HBox(
            [
                mb_thick_rg_widget,
                widgets.Label(value=labels["mb_thick_rg_widget"]),
            ]
        ),
        widgets.HBox(
            [
                mb_layer_s_rg_widget,
                widgets.Label(value=labels["mb_layer_s_rg_widget"]),
            ]
        ),
        widgets.HBox(
            [
                mb_max_ecc_widget,
                widgets.Label(value=labels["mb_max_ecc_widget"]),
            ]
        ),
        widgets.HBox(
            [
                mb_over_tol_widget,
                widgets.Label(value=labels["mb_over_tol_widget"]),
            ]
        ),
        widgets.HBox(
            [
                mb_min_rad_widget,
                widgets.Label(value=labels["mb_min_rad_widget"]),
            ]
        ),
        widgets.HBox(
            [
    ...
    display(
        widgets.VBox(widgets_and_labels), file_destination_widget, save_button
    )
    return (
        mb_type_widget,
        mb_occ_widget,
...
def widgets_helix_params():
    ...
    hlix_type_widget = widgets.Dropdown(
        options=options_hlix_type, value="actin", description="HLIX_TYPE:"
    )
    hlix_type_widget.style.description_width = "initial"

    hlix_mmer_rad_widget = widgets.BoundedFloatText(
        value=25, min=0, step=0.0001, max=10000, description="HLIX _MMER_RAD:"
    )
    hlix_mmer_rad_widget.style.description_width = "initial"

    hlix_pmer_l_widget = widgets.BoundedFloatText(
        value=1.2, min=0, step=0.0001, description="HLIX_PMER_L:"
    )
    hlix_pmer_l_widget.style.description_width = "initial"

    hlix_pmer_occ_widget = widgets.FloatRangeSlider(
        value=[0.001, 0.003],
        min=0,
        max=1,
        step=0.0001,
    ...
    hlix_min_p_len_widget = widgets.BoundedFloatText(
        value=177000,
        min=0,
        step=0.0001,
        max=1000000000000,
        description="HLIX_MIN_P_LEN:",
    )
    hlix_min_p_len_widget.style.description_width = "initial"

    hlix_hp_len_widget = widgets.BoundedFloatText(
        value=720,
        min=0,
        step=0.0001,
        max=100000000000,
        description="HLIX_HP_LEN:",
    )
    hlix_hp_len_widget.style.description_width = "initial"

    hlix_mz_len_widget = widgets.BoundedFloatText(
        value=50, min=0, step=0.0001, description="HLIX_MZ_LEN:"
    )
    hlix_mz_len_widget.style.description_width = "initial"

    hlix_mz_len_f_widget = widgets.BoundedFloatText(
        value=0.2, min=0, step=0.0001, max=1, description="HLIX_MZ_LEN_F:"
    )
    hlix_mz_len_f_widget.style.description_width = "initial"

    hlix_over_tol_widget = widgets.BoundedFloatText(
        value=1e-9,
        min=0,
        step=0.00000000001,
        max=1,
        description="HLIX_OVER_TOL:",
    )
    hlix_over_tol_widget.style.description_width = "initial"

    hlix_min_nmmer_widget = widgets.BoundedIntText(
        value=15, min=0, max=1000000000, description="HLIX_MIN_NMMER:"
    )
    hlix_min_nmmer_widget.style.description_width = "initial"

    a_bprop_widget = widgets.BoundedFloatText(
        value=0.5, max=1, min=0, description="A_BPROP:"
    )
    a_bprop_widget.style.description_width = "initial"

    a_max_p_branch_widget = widgets.BoundedFloatText(
        value=5, max=100000000, min=0, description="A_MAX_P_BRANCH:"
    )
    a_max_p_branch_widget.style.description_width = "initial"

    mt_rad_widget = widgets.BoundedFloatText(
        value=100.5, min=0, max=100000000, description="MT_RAD:"
    )
    mt_rad_widget.style.description_width = "initial"
    mt_rad_widget.disabled = True

    mt_nunits_widget = widgets.BoundedIntText(
        value=13, max=100000, min=0, description="MT_NUNITS:"
    )
    mt_nunits_widget.style.description_width = "initial"
    mt_nunits_widget.disabled = True

    file_destination_widget = FileChooser(
        os.getcwd(), title="Select the path and the name for the new file"
    )
    ...
    widgets_and_labels = [
        widgets.HBox(
            [hlix_type_widget, widgets.Label(value=labels["hlix_type_widget"])]
        ),
        widgets.HBox(
            [
                hlix_mmer_rad_widget,
                widgets.Label(value=labels["hlix_mmer_rad_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_pmer_l_widget,
                widgets.Label(value=labels["hlix_pmer_l_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_pmer_occ_widget,
                widgets.Label(value=labels["hlix_pmer_occ_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_min_p_len_widget,
                widgets.Label(value=labels["hlix_min_p_len_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_hp_len_widget,
                widgets.Label(value=labels["hlix_hp_len_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_mz_len_widget,
                widgets.Label(value=labels["hlix_mz_len_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_mz_len_f_widget,
                widgets.Label(value=labels["hlix_mz_len_f_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_over_tol_widget,
                widgets.Label(value=labels["hlix_over_tol_widget"]),
            ]
        ),
        widgets.HBox(
            [
                hlix_min_nmmer_widget,
                widgets.Label(value=labels["hlix_min_nmmer_widget"]),
            ]
        ),
        widgets.HBox(
            [a_bprop_widget, widgets.Label(value=labels["a_bprop_widget"])]
        ),
        widgets.HBox(
            [
                a_max_p_branch_widget,
                widgets.Label(value=labels["a_max_p_branch_widget"]),
            ]
        ),
        widgets.HBox(
            [mt_rad_widget, widgets.Label(value=labels["mt_rad_widget"])]
        ),
        widgets.HBox(
            [mt_nunits_widget, widgets.Label(value=labels["mt_nunits_widget"])]
        ),
    ]
...
def update_widgets_based_on_hlix(
    hlix_type,
    a_bprop_widget,
    a_max_p_branch_widget,
    mt_rad_widget,
...
def update_values_based_on_hlix(
    hlix_type,
    hlix_mmer_rad,
    hlix_pmer_l,
    hlix_pmer_occ,
...
def widgets_helix():
    ...
    display(
        widgets.VBox(
            [title, widgets.HBox([select_file_button, dropdown_1]), exec_button]
        )
    )
...
def widgets_add_app_files():
    ...
    display(
        widgets.VBox(
            [label, hbox_membrane, hbox_helix, hbox_proteins, hbox_mproteins]
        )
    )

    return (
        select_file_button_membrane,
        dropdown_membrane,
        select_file_button_helix,
        dropdown_helix,
        select_file_button_proteins,
        dropdown_proteins,
        select_file_button_mproteins,
        dropdown_mproteins,
    )
...
def widgets_show_files(list1, list2, list3, list4):
    ...
    """
    description_width = (
        "initial"  # Puedes ajustar este valor según tus necesidades
    )
    custom_width = "550px"

    dropdown_1 = widgets.Dropdown(
        options=list1,
        description="Membrane files:",
        style={"description_width": description_width},
        layout={"width": custom_width},
    )
    dropdown_2 = widgets.Dropdown(
        options=list2,
        description="Helix files:",
        style={"description_width": description_width},
        layout={"width": custom_width},
    )
    dropdown_3 = widgets.Dropdown(
        options=list3,
        description="Protein files:",
        style={"description_width": description_width},
        layout={"width": custom_width},
    )
    dropdown_4 = widgets.Dropdown(
        options=list4,
        description="Membrane protein files:",
        style={"description_width": description_width},
        layout={"width": custom_width},
    )
...
def widgets_exec_app():
    ...
    :return: tuple with widget created
    """
    widget_out_dir = FileChooser(
        os.getcwd(),
        show_only_dirs=True,
        title="Select where you want to save the output files:",
    )
    ntomos_widget = widgets.IntText(
        value=1, description="N_TOMOS (number of tomograms in the dataset):"
    )
    ntomos_widget.style.description_width = "initial"
    ntomos_widget.layout.width = "380px"

    voi_shape1 = widgets.BoundedIntText(
        value=400,
        min=0,
        max=100000,
    ...
    voi_size_widget = widgets.BoundedFloatText(
        value=10, min=0, description="VOI_VOXEL_SIZE (Voxel size, voxels/A):"
    )
    voi_size_widget.style.description_width = "initial"
    voi_size_widget.layout.width = "350px"

    mmer_tries_widget = widgets.BoundedFloatText(
        value=20,
        min=0,
    ...
    pmer_tries_widget = widgets.BoundedFloatText(
        value=100,
        min=0,
        max=1000,
    ...
    surf_dec_widget = widgets.BoundedFloatText(
        value=0.9,
        min=0,
    ...
    malign_mn_widget = widgets.BoundedFloatText(
        value=1,
        min=0,
    ...
    malign_mx_widget = widgets.BoundedFloatText(
        value=1.5,
        min=0,
    ...
    malign_sg_widget = widgets.BoundedFloatText(
        value=0.2,
        min=0,
    ...
    detector_snr_widget_low = widgets.BoundedFloatText(
        value=1,
        min=0,
        step=0.0001,
    ...
    detector_snr_widget_high = widgets.BoundedFloatText(
        value=2, min=0, step=0.0001
    )
    ...
    widget_min = widgets.BoundedIntText(
        value=-60,
        min=-1000,
        max=100000,
    ...
    voi_off_widget_1 = widgets.BoundedIntText(
        value=4, description="VOI_OFF (Empty halo, voxels):"
    )
    voi_off_label = widgets.Label(
        "this is the Start and End in voxels for the effective volume"
    )
    ...
    return (
        ...
        malign_mx_widget,
        malign_sg_widget,
        detector_snr_widget_low,
        detector_snr_widget_high,
        widget_min,
        widget_max,
        widget_paso,
        exec_button,
    ...
    for list in lists:
        if len(list) > 1:
            dropdown_widget = widgets.Dropdown(
                options=list, value=list[0], layout={"width": "550px"}
            )
        else:
            dropdown_widget = widgets.Dropdown(
                options=list, layout={"width": "550px"}
            )
