...
DETECTOR_SNR = [0.7, 1.3] # [0.35, 1.5]  # 0.2 # [.15, .25]
MALIGN_MN = 1
MALIGN_MX = 1.5
MALIGN_SG = 0.2
...
# OUTPUT LABELS
out_labels = dict(zip(PROTEINS_LIST, range(1, len(PROTEINS_LIST) + 1)))
...
for tomod_id in range(NTOMOS):
    ...
    for p_id, p_file in enumerate(MEMBRANES_LIST):
        ...
        if memb.get_type() == "sphere":
            ...
            mb_tor_generator = TorGen(radius_rg=unit_lbl(param_rg[0], param_rg[1]))
            set_mbs = SetMembranes(
                voi,
                VOI_VSIZE,
                mb_tor_generator,
