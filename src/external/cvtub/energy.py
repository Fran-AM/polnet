...
NOTE (polnet integration): The only modifications to this file are
converting absolute ``from cvtub.xxx`` imports to relative ``from .xxx``
imports so the package works when vendored under ``src/external/cvtub/``.
Each changed line is marked with ``# polnet: relative import``.
"""
import torch
dtype = torch.cuda.FloatTensor # if torch.cuda.is_available() else torch.FloatTensor

from .utils import double_W, double_W_prime, manual_softplus  # polnet: relative import (was ``from cvtub.utils ...``)
