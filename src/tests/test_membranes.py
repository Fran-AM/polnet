...
from polnet.samplegeneration.membranes.mb_ellipsoid import MbEllipsoid
from polnet.samplegeneration.membranes.mb_sphere import MbSphere
from polnet.samplegeneration.membranes.mb_torus import MbTorus

from polnet.utils.lio import write_mrc
...
class TestMembranes(unittest.TestCase):
    ...
    def test_mb_ellipsoid(self):
        ...
        write_mrc(
            ellipsoid.get_mask().astype(np.int8),
            self.output_path + "test_mb_ellipsoid_mask.mrc",
            v_size=self.v_size,
            dtype=np.int8,
        )
    ...
    def test_mb_sphere(self):
        ...
        write_mrc(
            sphere.get_mask().astype(np.int8),
            self.output_path + "test_mb_sphere_mask.mrc",
            v_size=self.v_size,
            dtype=np.int8,
        )
    ...
    def test_mb_torus(self):
        ...
        write_mrc(
            torus.get_mask().astype(np.int8),
            self.output_path + "test_mb_torus_mask.mrc",
            v_size=self.v_size,
            dtype=np.int8,
        )
