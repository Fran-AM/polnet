...
from .sample import SyntheticSample, MbFile, PnFile, FlmsFile, PmFile
...
class SynthTomo:
    ...
    Args:
        id: Tomogram index (1-based), used in output filenames.
        mbs_file_list: Relative paths to membrane .mbs config files.
        hns_file_list: Relative paths to helix .flms config files.
    ...
    def __init__(
        ...
        if not isinstance(shape, list) or not all(isinstance(s, int) for s in shape) or len(shape) != 3:
    ...
    def reset(self) -> None:
        ...
        if self._tomo_dir.exists() and self._tomo_dir.is_dir():
            ...
            logger.info(f"Output directory {self._tomo_dir} cleared.")
    ...
    def generate_sample(self) -> None:
        ...
        for mb_file_rpath in self._mbs_files:
            ...
            self._sample.add_set_membranes(
                params=mb_params,
                max_mbtries=10
            )
        ...
        for hn_file_rpath in self._hns_files:
            hn_file_apath = self._data_path / hn_file_rpath
            hn_file = FlmsFile()
            hn_params = hn_file.load(hn_file_apath)

            self._sample.add_helicoidal_network(
                params=hn_params
            )
        ...
        logger.info("Cytosolic proteins added. Adding membrane-bound proteins to the sample.")
    ...
    def simulate_tem(self) -> None:
        ...
        if self._tem_file_path is None:   
            logger.warning("No TEM config file provided. Skipping TEM simulation.")
            return
        ...
        self._temic.simulate(
            density=self._sample.density, 
            tem_params=tem_params,
            v_size=self._sample.v_size
        )
    
        logger.info(f"TEM simulation complete.")
    
    def save(self) ->  None:
        """Write all output files to tomo_dir.

        Outputs: density MRC, labels MRC, poly VTP, skel VTP, labels_table.csv.
        """
        ...
        # Save labels table and motif list
        self._save_labels_table()
        ...
        if self._sample.skel_vtp is not None:
            poly_skel_path = output_folder / f"tomo_{self._id:03d}_poly_skel.vtp"
            lio.save_vtp(
    ...
    def _save_labels_table(self) -> None:
        ...
        Returns:
            None
        """
        out_file = self._tomo_dir / "labels_table.csv"
        with open(out_file, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["MODEL", "LABEL"], delimiter="\t")
            writer.writeheader()
            for model_name, label_id in self._sample.label_registry:
                writer.writerow({"MODEL": model_name, "LABEL": label_id})
    ...
    def _save_motif_list(self) -> None:
        ...
        logger.info(f"Motif list ({len(motifs)} entries) saved to {out_file}")
